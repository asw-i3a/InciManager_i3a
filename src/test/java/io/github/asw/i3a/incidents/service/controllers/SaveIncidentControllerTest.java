...
public class SaveIncidentControllerTest {
	...
	public void saveNewIncidentTest() {
		...
		try {
			MvcResult reqResult = mockMvc.perform( request ).andExpect( status().isOk() )
					.andReturn();
			incident.set_id(
					new ObjectId( new JSONObject( reqResult.getResponse().getContentAsString() )
	...
	public void updateIncidentTest() {
		try {
			...
			MockHttpServletRequestBuilder request = post( "/incidents/" + indidenteID )
					.session( session ).contentType( MediaType.APPLICATION_JSON );
			MvcResult reqResult = mockMvc.perform( request ).andExpect( status().isOk() )
					.andReturn();
			Incident result = new ObjectMapper()
					.readValue( reqResult.getResponse().getContentAsString(), Incident.class );

			// Checking that the object we've find is the correct one.
			System.out.println( reqResult.getResponse().getContentAsString() );
			System.out.println( " original _id: " + incidente.get_id().toString()
					+ "; db incidentId: " + result.incidentId );
			...
			request = post( "/incidents/" + indidenteID ).session( session )
					.contentType( MediaType.APPLICATION_JSON );
			reqResult = mockMvc.perform( request ).andExpect( status().isOk() ).andReturn();
			Incident resultAfterInsert = new ObjectMapper()
					.readValue( reqResult.getResponse().getContentAsString(), Incident.class );
